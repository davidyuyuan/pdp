CREATE VIEW `prj-int-dev-covid19-nf-gls.datahub_metadata_dump.view_analysis_aspera_geoloc`
AS SELECT
    * EXCEPT(network_bin,
    mask),
FROM (
         SELECT
             *,
             NET.SAFE_IP_FROM_STRING(ip) & NET.IP_NET_MASK(4,
                                                           mask) AS network_bin
         FROM
             `prj-int-dev-covid19-nf-gls.datahub_metadata_dump.view_analysis_aspera` AS T1,
